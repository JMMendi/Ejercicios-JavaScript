...
function comprobarReal() {
    let candidato = parseFloat(document.getElementById("real").value);
    if (Number.isInteger(candidato)) {
        alert("Error, el número no es real.");
    } else {
        alert(candidato + " es real.");
    }
}
...
function comprobarCaracter() {
    let texto = document.getElementById("caracter").value;
    let caracter = texto.substring(0, 1);
    if (caracter.match("[A-Z]")) {
        alert("La primera letra en minúscula es: " + caracter.toLowerCase());
    } else if (caracter.match("[a-z]")) {
        alert("La primera letra en minúscula es: " + caracter.toUpperCase());
    }
}
...
function comprobarDNI() {
    ...
    if (candidatoDNI.length == 9) {
        //if (candidatoDNI.match('\d{8}\w{1}')) {
        let letra = candidatoDNI.substring(candidatoDNI.length, candidatoDNI.length - 1);
        let numero = parseInt(candidatoDNI.substring(0, candidatoDNI.length - 1));
